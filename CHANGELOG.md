...
* **Search History:** Automatically prepend the "استهبان‌من: " prefix to all user search queries saved in PocketBase.
### Changed
* **Navigation:** Streamlined the startup flow and configured the default/initial application route directly to `/home`.
* **Release:** Bumped project version to `v0.5.3` in `VERSION` and `pubspec.yaml`.
...
* **Release:** Bumped project version to `0.5.1` in `VERSION` and `pubspec.yaml`.
...
### Changed
* **Release:** Bumped project version to `0.5.0` in `VERSION` and `pubspec.yaml`.
